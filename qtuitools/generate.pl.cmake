...
my $qtuitools_headerlist = "";
my $qtuitools_headerlistpath = "";

$qtuitools_headerlist = "@CMAKE_CURRENT_SOURCE_DIR@/qtuitools_header_list";
$qtuitools_headerlistpath = "$here/$qtuitools_headerlist";

## If srcdir != builddir, use headerlist from src
$headerlistpath = $headerlist if ($headerlist =~ /^\//);
$qtuitools_headerlistpath = $qtuitools_headerlist if ($qtuitools_headerlist =~ /^\//);
...
open(HEADERS, $qtuitools_headerlistpath) or die "Couldn't open $qtuitools_headerlistpath: $!\n";
map { chomp ; $includes{$_} = 1 } <HEADERS>;
close HEADERS;
...
# Launch kalyptus
chdir "../smoke/qtuitools";
system "perl -I@kdebindings_SOURCE_DIR@/kalyptus @kdebindings_SOURCE_DIR@/kalyptus/kalyptus @ARGV --qt4 --globspace -fsmoke --name=qtuitools --classlist='@CMAKE_CURRENT_SOURCE_DIR@/classlist' --init-modules=qt $macros --no-cache --outputdir=$outdir @headers";
my $exit = $? >> 8;
exit $exit if ($exit);
chdir "$kalyptusdir";
