...
my $qtwebkit_headerlist = "";
my $qtwebkit_headerlistpath = "";

$qtwebkit_headerlist = "@CMAKE_CURRENT_SOURCE_DIR@/qtwebkit_header_list";
$qtwebkit_headerlistpath = "$here/$qtwebkit_headerlist";

## If srcdir != builddir, use headerlist from src
$headerlistpath = $headerlist if ($headerlist =~ /^\//);
$qtwebkit_headerlistpath = $qtwebkit_headerlist if ($qtwebkit_headerlist =~ /^\//);
...
open(HEADERS, $qtwebkit_headerlistpath) or die "Couldn't open $qtwebkit_headerlistpath: $!\n";
map { chomp ; $includes{$_} = 1 } <HEADERS>;
close HEADERS;
...
# Launch kalyptus
chdir "../smoke/qtwebkit";
system "perl -I@kdebindings_SOURCE_DIR@/kalyptus @kdebindings_SOURCE_DIR@/kalyptus/kalyptus @ARGV --qt4 --globspace -fsmoke --name=qtwebkit --classlist='@CMAKE_CURRENT_SOURCE_DIR@/classlist' --init-modules=qt $macros --no-cache --outputdir=$outdir @headers";
my $exit = $? >> 8;
exit $exit if ($exit);
chdir "$kalyptusdir";
