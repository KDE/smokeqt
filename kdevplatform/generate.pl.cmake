...
my $ktexteditorheaderlist = "@CMAKE_CURRENT_SOURCE_DIR@/ktexteditor_header_list";
my $ktexteditorheaderlistpath = "$here/$ktexteditorheaderlist";

my $threadweaverheaderlist = "@CMAKE_CURRENT_SOURCE_DIR@/threadweaver_header_list";
my $threadweaverheaderlistpath = "$here/$threadweaverheaderlist";

my $kdevplatformheaderlist = "@CMAKE_CURRENT_SOURCE_DIR@/kdevplatform_header_list";
my $kdevplatformheaderlistpath = "$here/$kdeheaderlist";
...
$ktexteditorheaderlistpath = $ktexteditorheaderlist if $ktexteditorheaderlist =~ /^\//;
$threadweaverheaderlistpath = $threadweaverheaderlist if $threadweaverheaderlist =~ /^\//;
$kdevplatformheaderlistpath = $kdevplatformheaderlist if $kdevplatformheaderlist =~ /^\//;
...
open(HEADERS, $ktexteditorheaderlistpath) or die "Couldn't open $ktexteditorheaderlistpath: $!\n";
map { chomp ; $kdeincludes{$_} = 1 unless /^\s*#/ } <HEADERS>;
close HEADERS;

open(HEADERS, $threadweaverheaderlistpath) or die "Couldn't open $threadweaverheaderlistpath: $!\n";
map { chomp ; $kdeincludes{$_} = 1 unless /^\s*#/ } <HEADERS>;
close HEADERS;

open(HEADERS, $kdevplatformheaderlistpath) or die "Couldn't open $kdevplatformheaderlistpath: $!\n";
map { chomp ; $kdeincludes{$_} = 1 unless /^\s*#/ } <HEADERS>;
close HEADERS;
...
# Launch kalyptus
chdir "../smoke/kde";
system "perl -I@kdebindings_SOURCE_DIR@/kalyptus @kdebindings_SOURCE_DIR@/kalyptus/kalyptus @ARGV --qt4 --globspace -fsmoke --name=kdevplatform --init-modules=qt,kde,ktexteditor --classlist=@CMAKE_CURRENT_SOURCE_DIR@/classlist $macros --no-cache --outputdir=$outdir @headers @kdeheaders";
my $exit = $? >> 8;
exit $exit if ($exit);
chdir "$kalyptusdir";
