...
my $sopranoheaderlist = "@CMAKE_CURRENT_SOURCE_DIR@/soprano_header_list";
my $sopranoheaderlistpath = "$here/$sopranoheaderlist";

my $nepomukheaderlist = "@CMAKE_CURRENT_SOURCE_DIR@/nepomuk_header_list";
my $nepomukheaderlistpath = "$here/$nepomukheaderlist";
...
$nepomukheaderlistpath = $nepomukheaderlist if $nepomukheaderlist =~ /^\//;
$sopranoheaderlistpath = $sopranoheaderlist if $sopranoheaderlist =~ /^\//;
...
open(HEADERS, $sopranoheaderlistpath) or die "Couldn't open $sopranoheaderlistpath: $!\n";
map { chomp ; $kdeincludes{$_} = 1 unless /^\s*#/ } <HEADERS>;
close HEADERS;

open(HEADERS, $nepomukheaderlistpath) or die "Couldn't open $nepomukheaderlistpath: $!\n";
map { chomp ; $kdeincludes{$_} = 1 unless /^\s*#/ } <HEADERS>;
close HEADERS;
...
# Launch kalyptus
chdir "../smoke/kde";
system "perl -I@kdebindings_SOURCE_DIR@/kalyptus @kdebindings_SOURCE_DIR@/kalyptus/kalyptus @ARGV --qt4 --globspace -fsmoke --name=nepomuk --init-modules=qt,soprano,kde --classlist=@CMAKE_CURRENT_SOURCE_DIR@/classlist $macros --no-cache --outputdir=$outdir @headers @kdeheaders";
my $exit = $? >> 8;
exit $exit if ($exit);
chdir "$kalyptusdir";
