...
my $qimageblitz_headerlist = "";
my $qimageblitz_headerlistpath = "";

$qimageblitz_headerlist = "@CMAKE_CURRENT_SOURCE_DIR@/qimageblitz_header_list";
$qimageblitz_headerlistpath = "$here/$qimageblitz_headerlist";

## If srcdir != builddir, use headerlist from src
$headerlistpath = $headerlist if ($headerlist =~ /^\//);
$qimageblitz_headerlistpath = $qimageblitz_headerlist if ($qimageblitz_headerlist =~ /^\//);
...
my %blitzincludes;
open(HEADERS, $qimageblitz_headerlistpath) or die "Couldn't open $qimageblitz_headerlistpath: $!\n";
map { chomp ; $blitzincludes{$_} = 1 } <HEADERS>;
close HEADERS;
...
my @blitzheaders = ();
my $blitzinc = @blitz_incs@;
-d $blitzinc or die "Couldn't process $blitzinc: $!\n";

find(
    {   wanted => sub {
            (-e || -l and !-d) and do {
                $f = substr($_, 1 + length $blitzinc);
                push ( @blitzheaders, $_ )
                  if( $blitzincludes{$f}        # Known header
                     && /\.h$/);     # Not a backup file etc. Only headers.
                undef $blitzincludes{$f}
             };
        },
        follow_fast => 1,
        follow_skip => 2,
...
# Launch kalyptus
chdir "../smoke/qtscript";
system "perl -I@kdebindings_SOURCE_DIR@/kalyptus @kdebindings_SOURCE_DIR@/kalyptus/kalyptus @ARGV --qt4 --globspace -fsmoke --name=qimageblitz --classlist='@CMAKE_CURRENT_SOURCE_DIR@/classlist' --init-modules=qt $macros --no-cache --outputdir=$outdir @headers @blitzheaders";
my $exit = $? >> 8;
exit $exit if ($exit);
chdir "$kalyptusdir";
