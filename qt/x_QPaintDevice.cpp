//Auto-generated by kalyptus. DO NOT EDIT.
#include <smoke.h>
#include <qt_smoke.h>
#include <qpaintdevice.h>

class x_QPaintDevice : public QPaintDevice {
public:
    static void x_0(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcNOP;
    }
    static void x_1(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawPoint;
    }
    static void x_2(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawFirst;
    }
    static void x_3(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcMoveTo;
    }
    static void x_4(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcLineTo;
    }
    static void x_5(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawLine;
    }
    static void x_6(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawRect;
    }
    static void x_7(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawRoundRect;
    }
    static void x_8(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawEllipse;
    }
    static void x_9(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawArc;
    }
    static void x_10(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawPie;
    }
    static void x_11(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawChord;
    }
    static void x_12(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawLineSegments;
    }
    static void x_13(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawPolyline;
    }
    static void x_14(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawPolygon;
    }
    static void x_15(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawCubicBezier;
    }
    static void x_16(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawText;
    }
    static void x_17(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawTextFormatted;
    }
    static void x_18(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawPixmap;
    }
    static void x_19(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawImage;
    }
    static void x_20(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawText2;
    }
    static void x_21(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawText2Formatted;
    }
    static void x_22(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcDrawLast;
    }
    static void x_23(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcBegin;
    }
    static void x_24(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcEnd;
    }
    static void x_25(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSave;
    }
    static void x_26(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcRestore;
    }
    static void x_27(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetdev;
    }
    static void x_28(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetBkColor;
    }
    static void x_29(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetBkMode;
    }
    static void x_30(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetROP;
    }
    static void x_31(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetBrushOrigin;
    }
    static void x_32(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetFont;
    }
    static void x_33(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetPen;
    }
    static void x_34(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetBrush;
    }
    static void x_35(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetTabStops;
    }
    static void x_36(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetTabArray;
    }
    static void x_37(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetUnit;
    }
    static void x_38(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetVXform;
    }
    static void x_39(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetWindow;
    }
    static void x_40(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetViewport;
    }
    static void x_41(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetWXform;
    }
    static void x_42(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetWMatrix;
    }
    static void x_43(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSaveWMatrix;
    }
    static void x_44(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcRestoreWMatrix;
    }
    static void x_45(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetClip;
    }
    static void x_46(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcSetClipRegion;
    }
    static void x_47(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcReservedStart;
    }
    static void x_48(Smoke::Stack x) {
	x[0].s_enum = (long)QPaintDevice::PdcReservedStop;
    }
    void x_49(Smoke::Stack x) const {
	// devType()
	int xret = this->QPaintDevice::devType();
	x[0].s_int = xret;
    }
    void x_50(Smoke::Stack x) const {
	// isExtDev()
	bool xret = this->QPaintDevice::isExtDev();
	x[0].s_bool = xret;
    }
    void x_51(Smoke::Stack x) const {
	// paintingActive()
	bool xret = this->QPaintDevice::paintingActive();
	x[0].s_bool = xret;
    }
    void x_52(Smoke::Stack x) {
	// setResolution(int)
	this->QPaintDevice::setResolution((int)x[1].s_int);
	(void)x; // noop (for compiler warning)
    }
    void x_53(Smoke::Stack x) const {
	// resolution()
	int xret = this->QPaintDevice::resolution();
	x[0].s_int = xret;
    }
    static void x_54(Smoke::Stack x) {
	// QPaintDevice(uint)
	x_QPaintDevice* xret = new x_QPaintDevice((uint)x[1].s_uint);
	x[0].s_class = (void*)xret;
    }
    x_QPaintDevice(uint x1) : QPaintDevice(x1) {
    }
    void x_55(Smoke::Stack x) {
	// cmd(int, QPainter*, QPDevCmdParam*)
	bool xret = this->QPaintDevice::cmd((int)x[1].s_int,(QPainter*)x[2].s_class,(QPDevCmdParam*)x[3].s_voidp);
	x[0].s_bool = xret;
    }
    void x_56(Smoke::Stack x) const {
	// metric(int)
	int xret = this->QPaintDevice::metric((int)x[1].s_int);
	x[0].s_int = xret;
    }
    void x_57(Smoke::Stack x) const {
	// fontMet(QFont*, int, const char*, int)
	int xret = this->QPaintDevice::fontMet((QFont*)x[1].s_class,(int)x[2].s_int,(const char*)x[3].s_voidp,(int)x[4].s_int);
	x[0].s_int = xret;
    }
    void x_58(Smoke::Stack x) const {
	// fontMet(QFont*, int, const char*)
	int xret = this->QPaintDevice::fontMet((QFont*)x[1].s_class,(int)x[2].s_int,(const char*)x[3].s_voidp);
	x[0].s_int = xret;
    }
    void x_59(Smoke::Stack x) const {
	// fontMet(QFont*, int)
	int xret = this->QPaintDevice::fontMet((QFont*)x[1].s_class,(int)x[2].s_int);
	x[0].s_int = xret;
    }
    void x_60(Smoke::Stack x) const {
	// fontInf(QFont*, int)
	int xret = this->QPaintDevice::fontInf((QFont*)x[1].s_class,(int)x[2].s_int);
	x[0].s_int = xret;
    }
    virtual bool cmd(int x1, QPainter* x2, QPDevCmdParam* x3) {
	Smoke::StackItem x[4];
	x[1].s_int = x1;
	x[2].s_class = (void*)x2;
	x[3].s_voidp = (void*)x3;
	if(qt_Smoke->binding->callMethod(6633, (void*)this, x)) return (bool)x[0].s_bool;
	return this->QPaintDevice::cmd(x1, x2, x3);
    }
    virtual int fontInf(QFont* x1, int x2) const {
	Smoke::StackItem x[3];
	x[1].s_class = (void*)x1;
	x[2].s_int = x2;
	if(qt_Smoke->binding->callMethod(6638, (void*)this, x)) return (int)x[0].s_int;
	return this->QPaintDevice::fontInf(x1, x2);
    }
    virtual int fontMet(QFont* x1, int x2, const char* x3, int x4) const {
	Smoke::StackItem x[5];
	x[1].s_class = (void*)x1;
	x[2].s_int = x2;
	x[3].s_voidp = (void*)x3;
	x[4].s_int = x4;
	if(qt_Smoke->binding->callMethod(6635, (void*)this, x)) return (int)x[0].s_int;
	return this->QPaintDevice::fontMet(x1, x2, x3, x4);
    }
    virtual int metric(int x1) const {
	Smoke::StackItem x[2];
	x[1].s_int = x1;
	if(qt_Smoke->binding->callMethod(6634, (void*)this, x)) return (int)x[0].s_int;
	return this->QPaintDevice::metric(x1);
    }
    virtual int resolution() const {
	Smoke::StackItem x[1];
	if(qt_Smoke->binding->callMethod(6631, (void*)this, x)) return (int)x[0].s_int;
	return this->QPaintDevice::resolution();
    }
    virtual void setResolution(int x1) {
	Smoke::StackItem x[2];
	x[1].s_int = x1;
	if(qt_Smoke->binding->callMethod(6630, (void*)this, x)) return;
	this->QPaintDevice::setResolution(x1);
    }
    static void xenum_operation(Smoke::EnumOperation xop, Smoke::Index xtype, void *&xdata, long &xvalue) {
	switch(xtype) {
	  case 406: //QPaintDevice::PDevCmd
	    switch(xop) {
	      case Smoke::EnumNew:
		xdata = (void*)new QPaintDevice::PDevCmd;
		break;
	      case Smoke::EnumDelete:
		delete (QPaintDevice::PDevCmd*)xdata;
		break;
	      case Smoke::EnumFromLong:
		*(QPaintDevice::PDevCmd*)xdata = (QPaintDevice::PDevCmd)xvalue;
		break;
	      case Smoke::EnumToLong:
		xvalue = (long)*(QPaintDevice::PDevCmd*)xdata;
		break;
	    }
	    break;
	}
    }
    ~x_QPaintDevice() { qt_Smoke->binding->deleted(220, (void*)this); }
};
void xenum_QPaintDevice(Smoke::EnumOperation xop, Smoke::Index xtype, void *&xdata, long &xvalue) {
    x_QPaintDevice::xenum_operation(xop, xtype, xdata, xvalue);
}
void xcall_QPaintDevice(Smoke::Index xi, void *obj, Smoke::Stack args) {
    x_QPaintDevice *xself = (x_QPaintDevice*)obj;
    switch(xi) {
	case 0: x_QPaintDevice::x_0(args);	break;
	case 1: x_QPaintDevice::x_1(args);	break;
	case 2: x_QPaintDevice::x_2(args);	break;
	case 3: x_QPaintDevice::x_3(args);	break;
	case 4: x_QPaintDevice::x_4(args);	break;
	case 5: x_QPaintDevice::x_5(args);	break;
	case 6: x_QPaintDevice::x_6(args);	break;
	case 7: x_QPaintDevice::x_7(args);	break;
	case 8: x_QPaintDevice::x_8(args);	break;
	case 9: x_QPaintDevice::x_9(args);	break;
	case 10: x_QPaintDevice::x_10(args);	break;
	case 11: x_QPaintDevice::x_11(args);	break;
	case 12: x_QPaintDevice::x_12(args);	break;
	case 13: x_QPaintDevice::x_13(args);	break;
	case 14: x_QPaintDevice::x_14(args);	break;
	case 15: x_QPaintDevice::x_15(args);	break;
	case 16: x_QPaintDevice::x_16(args);	break;
	case 17: x_QPaintDevice::x_17(args);	break;
	case 18: x_QPaintDevice::x_18(args);	break;
	case 19: x_QPaintDevice::x_19(args);	break;
	case 20: x_QPaintDevice::x_20(args);	break;
	case 21: x_QPaintDevice::x_21(args);	break;
	case 22: x_QPaintDevice::x_22(args);	break;
	case 23: x_QPaintDevice::x_23(args);	break;
	case 24: x_QPaintDevice::x_24(args);	break;
	case 25: x_QPaintDevice::x_25(args);	break;
	case 26: x_QPaintDevice::x_26(args);	break;
	case 27: x_QPaintDevice::x_27(args);	break;
	case 28: x_QPaintDevice::x_28(args);	break;
	case 29: x_QPaintDevice::x_29(args);	break;
	case 30: x_QPaintDevice::x_30(args);	break;
	case 31: x_QPaintDevice::x_31(args);	break;
	case 32: x_QPaintDevice::x_32(args);	break;
	case 33: x_QPaintDevice::x_33(args);	break;
	case 34: x_QPaintDevice::x_34(args);	break;
	case 35: x_QPaintDevice::x_35(args);	break;
	case 36: x_QPaintDevice::x_36(args);	break;
	case 37: x_QPaintDevice::x_37(args);	break;
	case 38: x_QPaintDevice::x_38(args);	break;
	case 39: x_QPaintDevice::x_39(args);	break;
	case 40: x_QPaintDevice::x_40(args);	break;
	case 41: x_QPaintDevice::x_41(args);	break;
	case 42: x_QPaintDevice::x_42(args);	break;
	case 43: x_QPaintDevice::x_43(args);	break;
	case 44: x_QPaintDevice::x_44(args);	break;
	case 45: x_QPaintDevice::x_45(args);	break;
	case 46: x_QPaintDevice::x_46(args);	break;
	case 47: x_QPaintDevice::x_47(args);	break;
	case 48: x_QPaintDevice::x_48(args);	break;
	case 49: xself->x_49(args);	break;
	case 50: xself->x_50(args);	break;
	case 51: xself->x_51(args);	break;
	case 52: xself->x_52(args);	break;
	case 53: xself->x_53(args);	break;
	case 54: x_QPaintDevice::x_54(args);	break;
	case 55: xself->x_55(args);	break;
	case 56: xself->x_56(args);	break;
	case 57: xself->x_57(args);	break;
	case 58: xself->x_58(args);	break;
	case 59: xself->x_59(args);	break;
	case 60: xself->x_60(args);	break;
	case 61: delete (QPaintDevice*)xself;	break;
    }
}
