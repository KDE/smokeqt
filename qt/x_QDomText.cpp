...
public:
	...
	x_QDomText* xret = new x_QDomText(*(const QDomText *)x[1].x_voidp);
	x[0].x_voidp = (void*)xret;
    }
	...
	QDomText& xret = this->QDomText::operator=(*(const QDomText *)x[1].x_voidp);
	x[0].x_voidp = (void*)&xret;
    }
    void x_3(xargs *x) {
	// splitText(int)
	QDomText xret = this->QDomText::splitText((int)x[1].x_int);
	...
	xargs x[1];
	if(call_method((void*)this, 2272, x)) return (QDomNode::NodeType)x[0].x_long;
...
void xcall_QDomText(xargs *x, void *s, int xi) {
    x_QDomText *xself = (x_QDomText*)s;
    switch(xi) {
	case 0: x_QDomText::x_0(x);	break;
	case 1: x_QDomText::x_1(x);	break;
