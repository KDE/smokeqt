//Auto-generated by kalyptus. DO NOT EDIT.
#include <smoke_types.h>
#include <qvariant.h>
#include <qglobal.h>
#include <qiodevice.h>
#include <qcstring.h>
#include <qhostaddress.h>
#include <qstring.h>
#include <qsocket.h>

class x_QSocket : public QSocket {
public:
    static void x_0(xargs *x) {
	x[0].x_long = (long)QSocket::ErrConnectionRefused;
    }
    static void x_1(xargs *x) {
	x[0].x_long = (long)QSocket::ErrHostNotFound;
    }
    static void x_2(xargs *x) {
	x[0].x_long = (long)QSocket::ErrSocketRead;
    }
    static void x_3(xargs *x) {
	x[0].x_long = (long)QSocket::Idle;
    }
    static void x_4(xargs *x) {
	x[0].x_long = (long)QSocket::HostLookup;
    }
    static void x_5(xargs *x) {
	x[0].x_long = (long)QSocket::Connecting;
    }
    static void x_6(xargs *x) {
	x[0].x_long = (long)QSocket::Connected;
    }
    static void x_7(xargs *x) {
	x[0].x_long = (long)QSocket::Closing;
    }
    static void x_8(xargs *x) {
	x[0].x_long = (long)QSocket::Connection;
    }
    static void x_9(xargs *x) {
	// QSocket(QObject*, const char*)
	x_QSocket* xret = new x_QSocket((QObject*)x[1].x_voidp,(const char*)x[2].x_voidp);
	x[0].x_voidp = (void*)xret;
    }
    x_QSocket(QObject* x1, const char* x2) : QSocket(x1, x2) {
    }
    static void x_10(xargs *x) {
	// QSocket(QObject*)
	x_QSocket* xret = new x_QSocket((QObject*)x[1].x_voidp);
	x[0].x_voidp = (void*)xret;
    }
    x_QSocket(QObject* x1) : QSocket(x1) {
    }
    static void x_11(xargs *x) {
	// QSocket()
	x_QSocket* xret = new x_QSocket();
	x[0].x_voidp = (void*)xret;
    }
    x_QSocket() : QSocket() {
    }
    void x_12(xargs *x) const {
	// state()
	QSocket::State xret = this->QSocket::state();
	x[0].x_long = (long)xret;
    }
    void x_13(xargs *x) const {
	// socket()
	int xret = this->QSocket::socket();
	x[0].x_int = (int)xret;
    }
    void x_14(xargs *x) {
	// setSocket(int)
	this->QSocket::setSocket((int)x[1].x_int);
    }
    void x_15(xargs *x) {
	// socketDevice()
	QSocketDevice* xret = this->QSocket::socketDevice();
	x[0].x_voidp = (void*)xret;
    }
    void x_16(xargs *x) {
	// setSocketDevice(QSocketDevice*)
	this->QSocket::setSocketDevice((QSocketDevice*)x[1].x_voidp);
    }
    void x_17(xargs *x) {
	// connectToHost(const QString&, Q_UINT16)
	this->QSocket::connectToHost(*(const QString *)x[1].x_voidp,*(Q_UINT16 *)x[2].x_voidp);
    }
    void x_18(xargs *x) const {
	// peerName()
	QString xret = this->QSocket::peerName();
	x[0].x_voidp = (void*)new QString(xret);
    }
    void x_19(xargs *x) {
	// open(int)
	bool xret = this->QSocket::open((int)x[1].x_int);
	x[0].x_bool = (bool)xret;
    }
    void x_20(xargs *x) {
	// close()
	this->QSocket::close();
    }
    void x_21(xargs *x) {
	// flush()
	this->QSocket::flush();
    }
    void x_22(xargs *x) const {
	// size()
	QIODevice::Offset xret = this->QSocket::size();
	x[0].x_voidp = (void*)new QIODevice::Offset(xret);
    }
    void x_23(xargs *x) const {
	// at()
	QIODevice::Offset xret = this->QSocket::at();
	x[0].x_voidp = (void*)new QIODevice::Offset(xret);
    }
    void x_24(xargs *x) {
	// at(QIODevice::Offset)
	bool xret = this->QSocket::at(*(QIODevice::Offset *)x[1].x_voidp);
	x[0].x_bool = (bool)xret;
    }
    void x_25(xargs *x) const {
	// atEnd()
	bool xret = this->QSocket::atEnd();
	x[0].x_bool = (bool)xret;
    }
    void x_26(xargs *x) const {
	// bytesAvailable()
	Q_ULONG xret = this->QSocket::bytesAvailable();
	x[0].x_voidp = (void*)new Q_ULONG(xret);
    }
    void x_27(xargs *x) const {
	// waitForMore(int)
	Q_ULONG xret = this->QSocket::waitForMore((int)x[1].x_int);
	x[0].x_voidp = (void*)new Q_ULONG(xret);
    }
    void x_28(xargs *x) const {
	// bytesToWrite()
	Q_ULONG xret = this->QSocket::bytesToWrite();
	x[0].x_voidp = (void*)new Q_ULONG(xret);
    }
    void x_29(xargs *x) {
	// readBlock(char*, Q_ULONG)
	Q_LONG xret = this->QSocket::readBlock((char*)x[1].x_voidp,*(Q_ULONG *)x[2].x_voidp);
	x[0].x_voidp = (void*)new Q_LONG(xret);
    }
    void x_30(xargs *x) {
	// writeBlock(const char*, Q_ULONG)
	Q_LONG xret = this->QSocket::writeBlock((const char*)x[1].x_voidp,*(Q_ULONG *)x[2].x_voidp);
	x[0].x_voidp = (void*)new Q_LONG(xret);
    }
    void x_31(xargs *x) {
	// readLine(char*, Q_ULONG)
	Q_LONG xret = this->QSocket::readLine((char*)x[1].x_voidp,*(Q_ULONG *)x[2].x_voidp);
	x[0].x_voidp = (void*)new Q_LONG(xret);
    }
    void x_32(xargs *x) {
	// getch()
	int xret = this->QSocket::getch();
	x[0].x_int = (int)xret;
    }
    void x_33(xargs *x) {
	// putch(int)
	int xret = this->QSocket::putch((int)x[1].x_int);
	x[0].x_int = (int)xret;
    }
    void x_34(xargs *x) {
	// ungetch(int)
	int xret = this->QSocket::ungetch((int)x[1].x_int);
	x[0].x_int = (int)xret;
    }
    void x_35(xargs *x) const {
	// canReadLine()
	bool xret = this->QSocket::canReadLine();
	x[0].x_bool = (bool)xret;
    }
    void x_36(xargs *x) {
	// readLine()
	QString xret = this->QSocket::readLine();
	x[0].x_voidp = (void*)new QString(xret);
    }
    void x_37(xargs *x) const {
	// port()
	Q_UINT16 xret = this->QSocket::port();
	x[0].x_voidp = (void*)new Q_UINT16(xret);
    }
    void x_38(xargs *x) const {
	// peerPort()
	Q_UINT16 xret = this->QSocket::peerPort();
	x[0].x_voidp = (void*)new Q_UINT16(xret);
    }
    void x_39(xargs *x) const {
	// address()
	QHostAddress xret = this->QSocket::address();
	x[0].x_voidp = (void*)new QHostAddress(xret);
    }
    void x_40(xargs *x) const {
	// peerAddress()
	QHostAddress xret = this->QSocket::peerAddress();
	x[0].x_voidp = (void*)new QHostAddress(xret);
    }
    void x_41(xargs *x) {
	// hostFound()
	this->QSocket::hostFound();
    }
    void x_42(xargs *x) {
	// connected()
	this->QSocket::connected();
    }
    void x_43(xargs *x) {
	// connectionClosed()
	this->QSocket::connectionClosed();
    }
    void x_44(xargs *x) {
	// delayedCloseFinished()
	this->QSocket::delayedCloseFinished();
    }
    void x_45(xargs *x) {
	// readyRead()
	this->QSocket::readyRead();
    }
    void x_46(xargs *x) {
	// bytesWritten(int)
	this->QSocket::bytesWritten((int)x[1].x_int);
    }
    void x_47(xargs *x) {
	// error(int)
	this->QSocket::error((int)x[1].x_int);
    }
    void x_48(xargs *x) {
	// sn_read(bool)
	this->QSocket::sn_read((bool)x[1].x_bool);
    }
    void x_49(xargs *x) {
	// sn_read()
	this->QSocket::sn_read();
    }
    void x_50(xargs *x) {
	// sn_write()
	this->QSocket::sn_write();
    }
    virtual QIODevice::Offset at() const {
	xargs x[1];
	if(call_method((void*)this, 6979, x)) {
	    QIODevice::Offset *xptr = (QIODevice::Offset *)x[0].x_voidp;
	    QIODevice::Offset xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QSocket::at();
    }
    virtual bool at(QIODevice::Offset x1) {
	xargs x[2];
	x[1].x_voidp = (void*)&x1;
	if(call_method((void*)this, 6980, x)) return (bool)x[0].x_bool;
	return this->QSocket::at(x1);
    }
    virtual bool atEnd() const {
	xargs x[1];
	if(call_method((void*)this, 6981, x)) return (bool)x[0].x_bool;
	return this->QSocket::atEnd();
    }
    virtual bool checkConnectArgs(const char* x1, const QObject* x2, const char* x3) {
	xargs x[4];
	x[1].x_voidp = (void*)x1;
	x[2].x_voidp = (void*)x2;
	x[3].x_voidp = (void*)x3;
	if(call_method((void*)this, 5475, x)) return (bool)x[0].x_bool;
	return this->QObject::checkConnectArgs(x1, x2, x3);
    }
    virtual void childEvent(QChildEvent* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5471, x)) return;
	this->QObject::childEvent(x1);
    }
    virtual void close() {
	xargs x[1];
	if(call_method((void*)this, 6976, x)) return;
	this->QSocket::close();
    }
    virtual void connectNotify(const char* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5473, x)) return;
	this->QObject::connectNotify(x1);
    }
    virtual void connectToHost(const QString& x1, Q_UINT16 x2) {
	xargs x[3];
	x[1].x_voidp = (void*)&x1;
	x[2].x_voidp = (void*)&x2;
	if(call_method((void*)this, 6973, x)) return;
	this->QSocket::connectToHost(x1, x2);
    }
    virtual void customEvent(QCustomEvent* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5472, x)) return;
	this->QObject::customEvent(x1);
    }
    virtual void disconnectNotify(const char* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5474, x)) return;
	this->QObject::disconnectNotify(x1);
    }
    virtual bool event(QEvent* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5410, x)) return (bool)x[0].x_bool;
	return this->QObject::event(x1);
    }
    virtual bool eventFilter(QObject* x1, QEvent* x2) {
	xargs x[3];
	x[1].x_voidp = (void*)x1;
	x[2].x_voidp = (void*)x2;
	if(call_method((void*)this, 5411, x)) return (bool)x[0].x_bool;
	return this->QObject::eventFilter(x1, x2);
    }
    virtual void flush() {
	xargs x[1];
	if(call_method((void*)this, 6977, x)) return;
	this->QSocket::flush();
    }
    virtual int getch() {
	xargs x[1];
	if(call_method((void*)this, 6988, x)) return (int)x[0].x_int;
	return this->QSocket::getch();
    }
    virtual void insertChild(QObject* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5433, x)) return;
	this->QObject::insertChild(x1);
    }
    virtual bool open(int x1) {
	xargs x[2];
	x[1].x_int = (int)x1;
	if(call_method((void*)this, 6975, x)) return (bool)x[0].x_bool;
	return this->QSocket::open(x1);
    }
    virtual QVariant property(const char* x1) const {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5447, x)) {
	    QVariant *xptr = (QVariant *)x[0].x_voidp;
	    QVariant xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QObject::property(x1);
    }
    virtual int putch(int x1) {
	xargs x[2];
	x[1].x_int = (int)x1;
	if(call_method((void*)this, 6989, x)) return (int)x[0].x_int;
	return this->QSocket::putch(x1);
    }
    virtual QByteArray readAll() {
	xargs x[1];
	if(call_method((void*)this, 3398, x)) {
	    QByteArray *xptr = (QByteArray *)x[0].x_voidp;
	    QByteArray xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QIODevice::readAll();
    }
    virtual Q_LONG readBlock(char* x1, Q_ULONG x2) {
	xargs x[3];
	x[1].x_voidp = (void*)x1;
	x[2].x_voidp = (void*)&x2;
	if(call_method((void*)this, 6985, x)) {
	    Q_LONG *xptr = (Q_LONG *)x[0].x_voidp;
	    Q_LONG xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QSocket::readBlock(x1, x2);
    }
    virtual QString readLine() {
	xargs x[1];
	if(call_method((void*)this, 6992, x)) {
	    QString *xptr = (QString *)x[0].x_voidp;
	    QString xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QSocket::readLine();
    }
    virtual Q_LONG readLine(char* x1, Q_ULONG x2) {
	xargs x[3];
	x[1].x_voidp = (void*)x1;
	x[2].x_voidp = (void*)&x2;
	if(call_method((void*)this, 6987, x)) {
	    Q_LONG *xptr = (Q_LONG *)x[0].x_voidp;
	    Q_LONG xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QSocket::readLine(x1, x2);
    }
    virtual void removeChild(QObject* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5434, x)) return;
	this->QObject::removeChild(x1);
    }
    virtual void setName(const char* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5416, x)) return;
	this->QObject::setName(x1);
    }
    virtual bool setProperty(const char* x1, const QVariant& x2) {
	xargs x[3];
	x[1].x_voidp = (void*)x1;
	x[2].x_voidp = (void*)&x2;
	if(call_method((void*)this, 5446, x)) return (bool)x[0].x_bool;
	return this->QObject::setProperty(x1, x2);
    }
    virtual void setSocket(int x1) {
	xargs x[2];
	x[1].x_int = (int)x1;
	if(call_method((void*)this, 6970, x)) return;
	this->QSocket::setSocket(x1);
    }
    virtual void setSocketDevice(QSocketDevice* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 6972, x)) return;
	this->QSocket::setSocketDevice(x1);
    }
    virtual QIODevice::Offset size() const {
	xargs x[1];
	if(call_method((void*)this, 6978, x)) {
	    QIODevice::Offset *xptr = (QIODevice::Offset *)x[0].x_voidp;
	    QIODevice::Offset xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QSocket::size();
    }
    virtual void sn_read(bool x1) {
	xargs x[2];
	x[1].x_bool = (bool)x1;
	if(call_method((void*)this, 7004, x)) return;
	this->QSocket::sn_read(x1);
    }
    virtual void sn_write() {
	xargs x[1];
	if(call_method((void*)this, 7006, x)) return;
	this->QSocket::sn_write();
    }
    virtual void timerEvent(QTimerEvent* x1) {
	xargs x[2];
	x[1].x_voidp = (void*)x1;
	if(call_method((void*)this, 5470, x)) return;
	this->QObject::timerEvent(x1);
    }
    virtual int ungetch(int x1) {
	xargs x[2];
	x[1].x_int = (int)x1;
	if(call_method((void*)this, 6990, x)) return (int)x[0].x_int;
	return this->QSocket::ungetch(x1);
    }
    virtual Q_LONG writeBlock(const char* x1, Q_ULONG x2) {
	xargs x[3];
	x[1].x_voidp = (void*)x1;
	x[2].x_voidp = (void*)&x2;
	if(call_method((void*)this, 6986, x)) {
	    Q_LONG *xptr = (Q_LONG *)x[0].x_voidp;
	    Q_LONG xret(*xptr);
	    delete xptr;
	    return xret;
	}
	return this->QSocket::writeBlock(x1, x2);
    }
    ~x_QSocket() {}
};
void xcall_QSocket(xargs *x, void *s, int xi) {
    x_QSocket *xself = (x_QSocket*)s;
    switch(xi) {
	case 0: x_QSocket::x_0(x);	break;
	case 1: x_QSocket::x_1(x);	break;
	case 2: x_QSocket::x_2(x);	break;
	case 3: x_QSocket::x_3(x);	break;
	case 4: x_QSocket::x_4(x);	break;
	case 5: x_QSocket::x_5(x);	break;
	case 6: x_QSocket::x_6(x);	break;
	case 7: x_QSocket::x_7(x);	break;
	case 8: x_QSocket::x_8(x);	break;
	case 9: x_QSocket::x_9(x);	break;
	case 10: x_QSocket::x_10(x);	break;
	case 11: x_QSocket::x_11(x);	break;
	case 12: xself->x_12(x);	break;
	case 13: xself->x_13(x);	break;
	case 14: xself->x_14(x);	break;
	case 15: xself->x_15(x);	break;
	case 16: xself->x_16(x);	break;
	case 17: xself->x_17(x);	break;
	case 18: xself->x_18(x);	break;
	case 19: xself->x_19(x);	break;
	case 20: xself->x_20(x);	break;
	case 21: xself->x_21(x);	break;
	case 22: xself->x_22(x);	break;
	case 23: xself->x_23(x);	break;
	case 24: xself->x_24(x);	break;
	case 25: xself->x_25(x);	break;
	case 26: xself->x_26(x);	break;
	case 27: xself->x_27(x);	break;
	case 28: xself->x_28(x);	break;
	case 29: xself->x_29(x);	break;
	case 30: xself->x_30(x);	break;
	case 31: xself->x_31(x);	break;
	case 32: xself->x_32(x);	break;
	case 33: xself->x_33(x);	break;
	case 34: xself->x_34(x);	break;
	case 35: xself->x_35(x);	break;
	case 36: xself->x_36(x);	break;
	case 37: xself->x_37(x);	break;
	case 38: xself->x_38(x);	break;
	case 39: xself->x_39(x);	break;
	case 40: xself->x_40(x);	break;
	case 41: xself->x_41(x);	break;
	case 42: xself->x_42(x);	break;
	case 43: xself->x_43(x);	break;
	case 44: xself->x_44(x);	break;
	case 45: xself->x_45(x);	break;
	case 46: xself->x_46(x);	break;
	case 47: xself->x_47(x);	break;
	case 48: xself->x_48(x);	break;
	case 49: xself->x_49(x);	break;
	case 50: xself->x_50(x);	break;
	case 51: delete (QSocket*)xself;	break;
    }
}
