...
public:
	...
	x_QCanvasSpline* xret = new x_QCanvasSpline((QCanvas*)x[1].x_voidp);
	...
	this->QCanvasSpline::setControlPoints(*(QPointArray *)x[1].x_voidp,(bool)x[2].x_bool);
    }
    void x_2(xargs *x) {
	// setControlPoints(QPointArray)
	this->QCanvasSpline::setControlPoints(*(QPointArray *)x[1].x_voidp);
    }
	...
	xargs x[1];
	if(call_method((void*)this, 642, x)) return (int)x[0].x_int;
...
void xcall_QCanvasSpline(xargs *x, void *s, int xi) {
    x_QCanvasSpline *xself = (x_QCanvasSpline*)s;
    switch(xi) {
	case 0: x_QCanvasSpline::x_0(x);	break;
