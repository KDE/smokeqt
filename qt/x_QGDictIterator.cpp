//Auto-generated by kalyptus. DO NOT EDIT.
#include <smoke_types.h>
#include <qgdict.h>
#include <qstring.h>

class x_QGDictIterator : public QGDictIterator {
public:
    static void x_0(xargs *x) {
	// QGDictIterator(const QGDict&)
	x_QGDictIterator* xret = new x_QGDictIterator(*(const QGDict *)x[1].x_voidp);
	x[0].x_voidp = (void*)xret;
    }
    x_QGDictIterator(const QGDict& x1) : QGDictIterator(x1) {
    }
    static void x_1(xargs *x) {
	// QGDictIterator(const QGDictIterator&)
	x_QGDictIterator* xret = new x_QGDictIterator(*(const QGDictIterator *)x[1].x_voidp);
	x[0].x_voidp = (void*)xret;
    }
    x_QGDictIterator(const QGDictIterator& x1) : QGDictIterator(x1) {
    }
    void x_2(xargs *x) {
	// operator=(const QGDictIterator&)
	QGDictIterator& xret = this->QGDictIterator::operator=(*(const QGDictIterator *)x[1].x_voidp);
	x[0].x_voidp = (void*)&xret;
    }
    void x_3(xargs *x) {
	// toFirst()
	QPtrCollection::Item xret = this->QGDictIterator::toFirst();
	x[0].x_voidp = (void*)new QPtrCollection::Item(xret);
    }
    void x_4(xargs *x) const {
	// get()
	QPtrCollection::Item xret = this->QGDictIterator::get();
	x[0].x_voidp = (void*)new QPtrCollection::Item(xret);
    }
    void x_5(xargs *x) const {
	// getKeyString()
	QString xret = this->QGDictIterator::getKeyString();
	x[0].x_voidp = (void*)new QString(xret);
    }
    void x_6(xargs *x) const {
	// getKeyAscii()
	const char* xret = this->QGDictIterator::getKeyAscii();
	x[0].x_voidp = (void*)xret;
    }
    void x_7(xargs *x) const {
	// getKeyInt()
	long xret = this->QGDictIterator::getKeyInt();
	x[0].x_long = (long)xret;
    }
    void x_8(xargs *x) const {
	// getKeyPtr()
	void* xret = this->QGDictIterator::getKeyPtr();
	x[0].x_voidp = (void*)xret;
    }
    void x_9(xargs *x) {
	// operator()()
	QPtrCollection::Item xret = this->QGDictIterator::operator()();
	x[0].x_voidp = (void*)new QPtrCollection::Item(xret);
    }
    void x_10(xargs *x) {
	// operator++()
	QPtrCollection::Item xret = this->QGDictIterator::operator++();
	x[0].x_voidp = (void*)new QPtrCollection::Item(xret);
    }
    void x_11(xargs *x) {
	// operator+=(uint)
	QPtrCollection::Item xret = this->QGDictIterator::operator+=((uint)x[1].x_uint);
	x[0].x_voidp = (void*)new QPtrCollection::Item(xret);
    }
    ~x_QGDictIterator() {}
};
void xcall_QGDictIterator(xargs *x, void *s, int xi) {
    x_QGDictIterator *xself = (x_QGDictIterator*)s;
    switch(xi) {
	case 0: x_QGDictIterator::x_0(x);	break;
	case 1: x_QGDictIterator::x_1(x);	break;
	case 2: xself->x_2(x);	break;
	case 3: xself->x_3(x);	break;
	case 4: xself->x_4(x);	break;
	case 5: xself->x_5(x);	break;
	case 6: xself->x_6(x);	break;
	case 7: xself->x_7(x);	break;
	case 8: xself->x_8(x);	break;
	case 9: xself->x_9(x);	break;
	case 10: xself->x_10(x);	break;
	case 11: xself->x_11(x);	break;
	case 12: delete (QGDictIterator*)xself;	break;
    }
}
