//Auto-generated by kalyptus. DO NOT EDIT.
#include <smoke_types.h>
#include <qpoint.h>
#include <qevent.h>

class x_QMouseEvent : public QMouseEvent {
public:
    static void x_0(xargs *x) {
	// QMouseEvent(QEvent::Type, const QPoint&, int, int)
	x_QMouseEvent* xret = new x_QMouseEvent((QEvent::Type)x[1].x_long,*(const QPoint *)x[2].x_voidp,(int)x[3].x_int,(int)x[4].x_int);
	x[0].x_voidp = (void*)xret;
    }
    x_QMouseEvent(QEvent::Type x1, const QPoint& x2, int x3, int x4) : QMouseEvent(x1, x2, x3, x4) {
    }
    static void x_1(xargs *x) {
	// QMouseEvent(QEvent::Type, const QPoint&, const QPoint&, int, int)
	x_QMouseEvent* xret = new x_QMouseEvent((QEvent::Type)x[1].x_long,*(const QPoint *)x[2].x_voidp,*(const QPoint *)x[3].x_voidp,(int)x[4].x_int,(int)x[5].x_int);
	x[0].x_voidp = (void*)xret;
    }
    x_QMouseEvent(QEvent::Type x1, const QPoint& x2, const QPoint& x3, int x4, int x5) : QMouseEvent(x1, x2, x3, x4, x5) {
    }
    void x_2(xargs *x) const {
	// pos()
	const QPoint& xret = this->QMouseEvent::pos();
	x[0].x_voidp = (void*)&xret;
    }
    void x_3(xargs *x) const {
	// globalPos()
	const QPoint& xret = this->QMouseEvent::globalPos();
	x[0].x_voidp = (void*)&xret;
    }
    void x_4(xargs *x) const {
	// x()
	int xret = this->QMouseEvent::x();
	x[0].x_int = (int)xret;
    }
    void x_5(xargs *x) const {
	// y()
	int xret = this->QMouseEvent::y();
	x[0].x_int = (int)xret;
    }
    void x_6(xargs *x) const {
	// globalX()
	int xret = this->QMouseEvent::globalX();
	x[0].x_int = (int)xret;
    }
    void x_7(xargs *x) const {
	// globalY()
	int xret = this->QMouseEvent::globalY();
	x[0].x_int = (int)xret;
    }
    void x_8(xargs *x) const {
	// button()
	Qt::ButtonState xret = this->QMouseEvent::button();
	x[0].x_long = (long)xret;
    }
    void x_9(xargs *x) const {
	// state()
	Qt::ButtonState xret = this->QMouseEvent::state();
	x[0].x_long = (long)xret;
    }
    void x_10(xargs *x) const {
	// stateAfter()
	Qt::ButtonState xret = this->QMouseEvent::stateAfter();
	x[0].x_long = (long)xret;
    }
    void x_11(xargs *x) const {
	// isAccepted()
	bool xret = this->QMouseEvent::isAccepted();
	x[0].x_bool = (bool)xret;
    }
    void x_12(xargs *x) {
	// accept()
	this->QMouseEvent::accept();
    }
    void x_13(xargs *x) {
	// ignore()
	this->QMouseEvent::ignore();
    }
    ~x_QMouseEvent() {}
};
void xcall_QMouseEvent(xargs *x, void *s, int xi) {
    x_QMouseEvent *xself = (x_QMouseEvent*)s;
    switch(xi) {
	case 0: x_QMouseEvent::x_0(x);	break;
	case 1: x_QMouseEvent::x_1(x);	break;
	case 2: xself->x_2(x);	break;
	case 3: xself->x_3(x);	break;
	case 4: xself->x_4(x);	break;
	case 5: xself->x_5(x);	break;
	case 6: xself->x_6(x);	break;
	case 7: xself->x_7(x);	break;
	case 8: xself->x_8(x);	break;
	case 9: xself->x_9(x);	break;
	case 10: xself->x_10(x);	break;
	case 11: xself->x_11(x);	break;
	case 12: xself->x_12(x);	break;
	case 13: xself->x_13(x);	break;
    }
}
